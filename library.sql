...
SELECT pg_catalog.setval('authors_books_id_seq', 2, true);
...
COPY books (id, title, status) FROM stdin;
\.
...
COPY checkouts (id, patron_id, book_id) FROM stdin;
\.
...
COPY patrons (id, first_name, last_name, phone_number) FROM stdin;
\.
